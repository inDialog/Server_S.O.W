...
float4 VolumeSampleColor( in float3 pos )
{
	...
	// iqs goodness
	float dif = clamp( (col.w - map( pos + 0.6*SUN_DIR ).w) / 0.2, 0.0, 0.2);
	float3 lin = float3(0.8, 0.9,1)*1.2 + 1*float3(0.1, 0.5, 0.5)*dif;
	col.xyz *= col.xyz;
	col.xyz *= lin;
	col.a *= 0.15;
	col.rgb *= col.a;
...
float3 skyColor( in float3 ro, in float3 rd )
{
	...
	//hor += 0.5*float3(0, 0, 0); //*exp2(hort*1.-1.);
	//col += hor;
